<div>
    <x-button.secondary wire:click="$toggle('showModal')" class="flex items-center space-x-2 mt-4"><x-icon.upload class="text-cool-gray-500"/> <span>Import</span></x-button.secondary>
            ...
            <x-slot name="content">
                ...
                <div>
                    ...
                    <x-input.group for="date" label="Date">
                        <x-input.select wire:model="fieldColumnMap.date" id="date">
                            <option value="" disabled>Select Column...</option>
                            @foreach ($columns as $column)
                                <option>{{ $column }}</option>
                            @endforeach
